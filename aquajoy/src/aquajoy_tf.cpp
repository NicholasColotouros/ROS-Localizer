...
//#define FEED_FORWARD_DEPTH_CONTROL
...
class AquaTeleopJoy {
public:
  AquaTeleopJoy(): nh("~"), lastStateUpdateT(ros::Time()), eStopOn(false), dyncfgSyncRequest(false), useFlexibleSineGait(false) {
    ...
    std::string aquaStateTopic = nh.resolveName("aqua_state");
    ...
    if (aquaStateTopic == nh.getNamespace() + "/aqua_state") aquaStateTopic = "/aqua/state";
    ...
    stateSub = nh.subscribe<aquacore::StateMsg>(aquaStateTopic, 1, &AquaTeleopJoy::aquaStateCallback, this);
    ...
    nh.param<bool>("use_flexible_sine_gait", useFlexibleSineGait, useFlexibleSineGait);
    #define PARAM(v, d) nh.param<double>(#v, params.v, d)
    PARAM(joy_axis_deadzone, 0.15);
    ...
    PARAM(default_fixed_depth, -1.0);
    PARAM(max_roll_vel, 450.0);
    PARAM(max_pitch_vel, 450.0);
    PARAM(max_yaw_vel, 900.0);
    PARAM(max_depth_vel, 3.0);
    #undef PARAM
    paramsMutex.unlock();
...
protected:
  void runAquaStartupSequence() {
    ...
    if (useFlexibleSineGait) {
      setGaitSrv.request.gait = "flexible-sine";
    } else {
    ...
    // Explicitly request for Aqua's yaw and depth state
    // NOTE: we assume that the robot is up-right, so we should trust the state data from /aqua/state
    if (getStateCln.call(getStateTyp)) {
      state.yaw_pos = getStateTyp.response.state.YawAngle;
      if (params.default_fixed_depth < 0) {
        state.depth_pos = getStateTyp.response.state.Depth;
      } else {
        state.depth_pos = params.default_fixed_depth;
      }
    } else {
  ...
  void aquaStateCallback(const aquacore::StateMsg::ConstPtr& msg) {
    // WARNING: Do not use the yaw angle from the state since it is left-hand oriented, but rather use the one given by TF (see getAquaCurrentTF)
    //getStateTyp.response.state.YawAngle = msg->YawAngle;
    getStateTyp.response.state.Depth = msg->Depth;
  };


  bool getAquaCurrentTF() {
    tf::StampedTransform transform;
    double rollRad, pitchRad, yawRad;
    try {
      tfListener.lookupTransform("/latest_fix", "/aqua_base", ros::Time(0), transform);
      tf::Quaternion robotRotationGlobalFrame = transform.getRotation();
      tf::Matrix3x3(robotRotationGlobalFrame).getEulerYPR(yawRad, pitchRad, rollRad);
      getStateTyp.response.state.RollAngle = rollRad/DEGREE;
      getStateTyp.response.state.PitchAngle = pitchRad/DEGREE;
      getStateTyp.response.state.YawAngle = yawRad/DEGREE;
      return true;
  ...
  bool updateAPMode(int mode) {
    ...
    if (apModeCln.call(apModeTyp)) {
      ROS_INFO_STREAM("Set AP mode to " << apModeTyp.request.mode << ", received " << (int) apModeTyp.response.response);
      return true;
    } else {
  ...
  bool updateParams(UpdateAquaTeleopJoyParams::Request& req, UpdateAquaTeleopJoyParams::Response& res) {
    ...
    UPDATE_PARAM(default_fixed_depth);
  ...
  void joyCallback(const sensor_msgs::Joy::ConstPtr& joy) {
    ...
    if ((state.ctrl_mode != AquaTeleopJoyState::CTRL_MODE_RAW) &&
      ...
      state.ctrl_mode = AquaTeleopJoyState::CTRL_MODE_RAW;
      updateAPMode(aquacore::AutopilotModes::AP_OFF);
      ROS_INFO_STREAM("Switched to " << toStr(state.ctrl_mode));
    ...
    switch (state.ctrl_mode) {
      ...
      default:
        ...
        state.raw_cmd.yaw = joypad.LX*params.max_yaw_cmd;
        state.raw_cmd.roll = joypad.RX*params.max_roll_cmd;
        if (state.modifier) {
      ...
      case AquaTeleopJoyState::CTRL_MODE_GLOBAL_RP_POS:
        state.raw_cmd.speed = joypad.LY*params.max_speed_cmd;
        state.yaw_vel = -joypad.LX*params.max_yaw_vel;
        if (state.modifier) {
          ...
          state.roll_pos = joypad.RX*params.max_roll_pos;
      ...
      case AquaTeleopJoyState::CTRL_MODE_GLOBAL_RP_VEL_D:
        state.raw_cmd.speed = joypad.LY*params.max_speed_cmd;
        state.yaw_vel = -joypad.LX*params.max_yaw_vel;
        state.roll_vel = joypad.RX*params.max_roll_vel;
        if (state.modifier) {
          state.pitch_vel = 0;
          state.raw_cmd.heave = 0;
          state.depth_vel = -joypad.RY*params.max_depth_vel;
  ...
  void publishCtrl(const ros::TimerEvent& e) {
    // Terminate if in E-Stop mode
    if (eStopOn || !ros::ok()) {
      return;
    }
    
    // Send raw command in raw mode
    if (state.ctrl_mode == AquaTeleopJoyState::CTRL_MODE_RAW) {
      cmdPub.publish(state.raw_cmd);
    } else {
      // Get Aqua's latest orientation
      if (!getAquaCurrentTF()) {
        return;
      }
      ...
      if (state.ctrl_mode == AquaTeleopJoyState::CTRL_MODE_GLOBAL_RP_POS) {
        state.yaw_pos = getStateTyp.response.state.YawAngle;
      } else if (state.ctrl_mode == AquaTeleopJoyState::CTRL_MODE_GLOBAL_RP_POS_D) {
        state.yaw_pos = getStateTyp.response.state.YawAngle;
#ifdef FEED_FORWARD_DEPTH_CONTROL
        state.depth_pos = getStateTyp.response.state.Depth;
#endif
      } else if (state.ctrl_mode == AquaTeleopJoyState::CTRL_MODE_GLOBAL_RP_VEL_D) {
        state.roll_pos = getStateTyp.response.state.RollAngle;
        state.pitch_pos = getStateTyp.response.state.PitchAngle;
        state.yaw_pos = getStateTyp.response.state.YawAngle;
#ifdef FEED_FORWARD_DEPTH_CONTROL
        state.depth_pos = getStateTyp.response.state.Depth;
#endif
      ...
      cmd.pose.position.x = state.raw_cmd.speed;
      cmd.pose.position.y = state.raw_cmd.heave;
      cmd.pose.position.z = state.depth_pos;
      cmd.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(state.roll_pos*DEGREE, state.pitch_pos*DEGREE, state.yaw_pos*DEGREE);
  ...
  bool useFlexibleSineGait; // otherwise, use hover-midoff gait
};
